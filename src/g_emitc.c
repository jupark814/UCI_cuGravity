...
inst_mul1(const struct g__ann_program_inst *inst, FILE *file)
{
	if (inst->cuda_inst) {
		if (P(file,
			...
			UL(inst->arg[2].i)) ||
			P(file,
			"    int size_A = %lu * %lu * sizeof(float);\n"
			"    int size_B = %lu * sizeof(float);\n"
			"    int size_z = %lu * sizeof(float);\n"
			"    float *deviceA;\n"
			"    float *deviceB;\n"
			"    float *deviceOutput;\n"
...
inst_suby(const struct g__ann_program_inst *inst, FILE *file)
{
	if (inst->cuda_inst) {
		if (P(file,
			...
			"    cudaMalloc((void**) &devicey_, size);\n"
			"    cudaMalloc((void**) &devicez, size);\n"
			"    cudaMemcpy(deviceA, A, size, cudaMemcpyHostToDevice);\n"
			"    cudaMemcpy(devicey_, y_, size, cudaMemcpyHostToDevice);\n",
			precision(inst),
			precision(inst),
			UL(inst->arg[0].i),
			precision(inst),
	...
	if (inst->cuda_inst) {
		if (P(file,
		  ...
		  P(file,
			"    dim3 DimGrid((%lu+255)/256,1,1);\n"
			"    dim3 DimBlock(256,1,1);\n"
			"    _CURELUD_<<<DimGrid, DimBlock>>>(deviceA, deviceB, %lu);\n"
			"    cudaDeviceSynchronize();\n"
			"    cudaMemcpy(za, deviceA, size_A, cudaMemcpyDeviceToHost);\n"
			"    cudaFree(deviceA);\n"
			"    cudaFree(deviceB);\n"
...
cudaFunction(const struct g__ann *ann, FILE *file)
{
	if (ann->cuda) {
		if (P(file,
			...
			P(file,
				"/* _CURELUD_ */\n"
				"__global__ void _CURELUD_(float *za, float *B, int n) {\n"
				"  int i = blockIdx.x * blockDim.x + threadIdx.x;\n"
				"  if (i < n) {\n"
				"    if (0.0f >= B[i]) {\n"
				"      za[i] = 0.0f;\n"
			...
			P(file,
				"/* _CUSUBY_ */\n"
				"__global__ void _CUSUBY_(float *za, float *A, float *Y, int n) {\n"
				"  int i = blockIdx.x * blockDim.x + threadIdx.x;\n"
				"  if (i < n) {\n"
				"    za[i] = A[i] - Y[i];\n"
				"  }\n"
